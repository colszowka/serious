...
default this value is '~' which can be customized. If you do the change is global so you will 
have update existing articles that have summaries to use the new delimiter. You can define a unique 
summary delimiter per article if needed otherwise Serious fallback on the default configuration. 
You can change the global summary delimiter with:
